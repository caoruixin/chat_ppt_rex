...
def print_layouts_and_placeholders(prs):
    for index, layout in enumerate(prs.slide_layouts):
        print(f"Layout {index}: {layout.name}")
        for shape in layout.placeholders:
            print(f"  - Placeholder {shape.placeholder_format.idx}: {shape.name} (type: {shape.placeholder_format.type})")
        print("-" * 40)
...
def main():
    ...
    ## New ChatPPT Demo [Title]

    ## 2024 业绩概述 [Comparison]
    - 总收入增长15%
    - 市场份额扩大至30%
    ...
    print_layouts(prs)
    print_layouts_and_placeholders(prs)
